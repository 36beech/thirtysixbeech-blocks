...
if (!name) {
  console.error("Usage: npm run createblock -- <block-slug>");
  process.exit(1);
}

const blockDir = `src/${name}`;
const indexFile = join(blockDir, "index.js");
const saveFile = join(blockDir, "save.js");
const iconFile = join(blockDir, "icon.svg");

// 1) Run create-block
const args = ["@wordpress/create-block@latest", name, "--namespace=thirtysixbeech-blocks", "--no-plugin", "--variant=dynamic", `--target-dir=${blockDir}`];
const result = spawnSync("npx", args, { stdio: "inherit" });
if (result.status !== 0) process.exit(result.status ?? 1);

// 2) Ensure save.js exists
...
if (!existsSync(iconFile)) {
  // paste your default-block.svg contents here ↓
...
console.log("✅ Block scaffolded with save.js, icon.svg, and index.js updated.");
